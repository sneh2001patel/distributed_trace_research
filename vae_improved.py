...
class GraphEncoderVAE(nn.Module):
    ...
    def build_feats(self, x: torch.Tensor) -> torch.Tensor:
        ...
        duration = _log_normalize_duration(x[:, 2], self.duration_mean, self.duration_std)
...
class GNNDecoder(nn.Module):
    def __init__(
        ...
        self.n_pod_classes = n_pod_classes
        ...
        self.pod_head = nn.Linear(hidden_dim, n_pod_classes)
        self.op_head = nn.Linear(hidden_dim, n_op_classes)
    ...
    def decode_for_training(self, z_graph: torch.Tensor, z_nodes: torch.Tensor, enc_h: torch.Tensor):
        """
        z_graph: (latent_dim,)
        ...
        return pod_logits, op_logits, duration_pred, edge_logits
...
def edge_recon_loss(real_edges: torch.Tensor, pred_edge_logits: torch.Tensor, n_nodes: int, device: torch.device,
                    neg_weight: float = 2.0) -> torch.Tensor:
    ...
    if num_pos == 0:
        ...
        pos_weight = torch.tensor(min((num_neg / (num_pos + 1e-6)).sqrt().item(), 5.0), device=device)
...
def total_vae_loss(real_data,
                   pod_logits, op_logits, duration_pred,
                   pred_edge_logits,
                   mu_g, logvar_g,
                   mu_n, logvar_n,
...
@torch.no_grad()
def evaluate_reconstruction(dataset, encoder, decoder, device, edge_threshold: float = 0.75):
    encoder.eval()
    decoder.eval()
    ...
    for data in DataLoader(dataset, batch_size=1, shuffle=False):
        ...
        pod_logits, op_logits, duration_pred, edge_logits = decoder.decode_for_training(z_g[0], z_n, h_enc)

        pod_pred = pod_logits.argmax(dim=1)
        op_pred = op_logits.argmax(dim=1)
        dur_pred = decode_and_denorm_duration(duration_pred, encoder.duration_mean, encoder.duration_std)

        pod_real = data.x[:, 0].long()
