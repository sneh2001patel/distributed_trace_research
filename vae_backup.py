...
class GraphEncoderVAE(nn.Module):
    ...
    def build_feats(self, x: torch.Tensor) -> torch.Tensor:
        pod_ids = (
            x[:, 0].long().clamp(min=0, max=self.pod_embeddings.num_embeddings - 1)
        )
...
class GNNDecoder(nn.Module):
    def __init__(
        ...
        self.pod_head = nn.Sequential(
            nn.Linear(hidden_dim, head_hidden_dim),
            nn.LayerNorm(head_hidden_dim),
            nn.GELU(),
            nn.Dropout(p=head_dropout),
            nn.Linear(head_hidden_dim, n_pod_classes),
        )
    ...
    def decode_for_training(
        ...
        pod_logits = self.pod_head(h_cls)
