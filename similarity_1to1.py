...
def main():
    ...
    for i, (real_graph, synth_graph) in enumerate(zip(real_ds, synth_ds)):
        ...
        # out = compute_node_attr_similarity_with_jsd(G_synth, G_real)
        # print(out["combined"], out["match_score"], out["jsd_score"], out["details"])
        # f"Node Attr Sim: {node_attr_similarity:6.2f}% | "
        # f"Iso(struct): {fidelity['isomorphic_structure']} | "
        # f"Iso(attr): {fidelity['isomorphic_attr']}"
    ...
    # avg_node_attr_similarity = sum(
    #     s["node_attr_similarity"] for s in similarities
    # ) / len(similarities)

    print(f"Average Overall Similarity: {avg_similarity:6.2f}%")
    # print(f"Average Node Attribute Similarity: {avg_node_attr_similarity:6.2f}%")
    ...
    # best_node_attr = max(similarities, key=lambda x: x["node_attr_similarity"])
    # worst_node_attr = min(similarities, key=lambda x: x["node_attr_similarity"])
    ...
    # print(
    #     f"\nMost (Node Attr):  Graph {best_node_attr['index']} "
    #     f"({best_node_attr['node_attr_similarity']:.2f}% similar)"
    # )
    # print(
    #     f"Least (Node Attr): Graph {worst_node_attr['index']} "
    #     f"({worst_node_attr['node_attr_similarity']:.2f}% similar)"
    # )
    ...
    # print(f"\nReconstruction Quality (Node Attr Similarity):")
    # node_excellent = sum(1 for s in similarities if s["node_attr_similarity"] >= 80)
    # node_good = sum(1 for s in similarities if 50 <= s["node_attr_similarity"] < 80)
    # node_poor = sum(1 for s in similarities if s["node_attr_similarity"] < 50)
    # print(
    #     f"  (≥80% similarity): {node_excellent}/{len(similarities)} "
    #     f"({node_excellent/len(similarities)*100:.1f}%)"
    # )
    # print(
    #     f" (50-80% similarity):    {node_good}/{len(similarities)} "
    #     f"({node_good/len(similarities)*100:.1f}%)"
    # )
    # print(
    #     f" (<50% similarity):      {node_poor}/{len(similarities)} "
    #     f"({node_poor/len(similarities)*100:.1f}%)"
    # )
